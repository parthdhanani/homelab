...
[ -z "${SNAP_ID:-}" ] && { /home/ubuntu/.claude/scripts/notify.sh "restore spot-check FAILED" "No kopia snapshot found — backups may be broken."; exit 1; }
...
[ -z "${FILE:-}" ] && { /home/ubuntu/.claude/scripts/notify.sh "restore spot-check FAILED" "Snapshot $SNAP_ID listed no files."; exit 1; }
...
if [ "${SIZE:-0}" -gt 0 ] 2>/dev/null; then
    ...
    /home/ubuntu/.claude/scripts/notify.sh "restore spot-check FAILED" "Restored '$FILE' from $SNAP_ID but got ${SIZE:-0} bytes — remote data may be corrupt. Investigate before trusting backups."
    exit 1
fi
