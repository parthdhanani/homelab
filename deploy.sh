...
mkdir -p "${COMPOSE_DIR}/data"/{postgres,moodle,moodledata,vaultwarden,n8n,adguard/work,adguard/conf,tianji,kopia/repository,kopia/config,kopia/cache,kopia/logs,traxlrs,workstation/{claude,workspace,gws-config,vault},tailscale,portfolio,forgejo,aquasoul,actualbudget,moodle-uploads}
# ActualBudget: pre-create subdirs, owned by node user (uid 1000)
# actual-server image runs as node (uid 1000) — chown is correct here
...
if [ -n "${AQUASOUL_DOMAIN:-}" ]; then
    AQUASOUL_DOMAIN="${AQUASOUL_DOMAIN}" envsubst '${AQUASOUL_DOMAIN}' \
        < "${COMPOSE_DIR}/configs/nginx/aquasoul.conf.template" \
        > "${COMPOSE_DIR}/configs/nginx/aquasoul.conf"
    echo "  aquasoul.conf → ${AQUASOUL_DOMAIN}"
else
...
# ActualBudget: already handled above via mkdir + chown 1000
...
# PgBouncer requires md5 passwords: "md5" + md5(password + username)
# Generated here so secrets never sit in the image or config file
...
md5pgpass() {
    local user="$1" pass="$2"
    printf 'md5%s' "$(printf '%s%s' "$pass" "$user" | md5sum | cut -d' ' -f1)"
}

cat > "${PGBOUNCER_CONF_DIR}/userlist.txt" <<USERLIST
"${MOODLE_DB_USER}" "$(md5pgpass "$MOODLE_DB_USER" "$MOODLE_DB_PASSWORD")"
"${N8N_DB_USER}" "$(md5pgpass "$N8N_DB_USER" "$N8N_DB_PASSWORD")"
"${TIANJI_DB_USER}" "$(md5pgpass "$TIANJI_DB_USER" "$TIANJI_DB_PASSWORD")"
"${TRAXLRS_DB_USER}" "$(md5pgpass "$TRAXLRS_DB_USER" "$TRAXLRS_DB_PASSWORD")"
"${FORGEJO_DB_USER}" "$(md5pgpass "$FORGEJO_DB_USER" "$FORGEJO_DB_PASSWORD")"
"${MINIFLUX_DB_USER:-miniflux_user}" "$(md5pgpass "${MINIFLUX_DB_USER:-miniflux_user}" "$MINIFLUX_DB_PASSWORD")"
"pgbouncer_admin" "$(md5pgpass "pgbouncer_admin" "$POSTGRES_PASSWORD")"
USERLIST
chmod 644 "${PGBOUNCER_CONF_DIR}/userlist.txt"
echo "  userlist.txt generated (7 users)"

# ── Build custom images ──

echo "Building custom images..."
cd "$COMPOSE_DIR"
docker compose build moodle workstation traxlrs n8n
...
0 4 * * 0 docker system prune -af --volumes --filter 'label!=cryptex' >> /var/log/cryptex-prune.log 2>&1
...
# Health check every 5 minutes (silent unless failures)
*/5 * * * * /opt/cryptex/scripts/health-check.sh > /dev/null 2>&1 || echo \"[FAIL] \$(date)\" >> /var/log/cryptex-health.log
...
echo "code.${DOMAIN}          → http://cryptex-workstation:3000"
...
echo "════════════════════════════════════════════════════"
echo ""
echo "Claude Code is installed in the workstation container."
echo "All your skills, hooks, and memory are pre-loaded."
echo ""
echo "1. Open the workstation terminal:"
echo "   https://code.${DOMAIN}"
echo ""
echo "2. Run this command:"
echo "   claude login"
echo ""
echo "3. Copy the URL it prints → paste into your browser → log in with Claude Pro"
echo "4. Paste the auth code back into the terminal"
echo "5. Token saved at /root/.claude — persists across container restarts"
echo ""
echo "After login, test it:"
echo "  claude -p 'list all running docker containers and their status'"
echo ""
...
svc_enabled "workstation" && echo "  [ ] Claude Code — run 'claude login' at https://code.${DOMAIN}"
