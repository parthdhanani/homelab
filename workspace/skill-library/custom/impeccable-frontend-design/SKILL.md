...
**Sub-skills available in `references/`** — load when doing targeted work:
`adapt` `animate` `arrange` `audit` `bolder` `clarify` `colorize` `critique` `delight` `distill` `extract` `harden` `normalize` `onboard` `optimize` `overdrive` `polish` `quieter` `typeset`
Example: `cat ~/.claude/skill-library/custom/impeccable-frontend-design/references/animate.md`
