...
**Stack:** 24-container Docker Compose + Zellij host service on Oracle Cloud ARM64 (Ubuntu)
...
| cryptex-n8n | Workflow automation (official image; qpdf/pdftotext injected via n8n-tools init container into n8n_tools volume) |
...
| zellij (host) | Web terminal — Zellij 0.44 + Claude Code + Gemini CLI + yazi + starship + zoxide. Systemd service on host, not a container. Port 8082. |
...
| cryptex-rclone | WebDAV file server — iOS Obsidian + Finder mount via Tailscale (files.DOMAIN) |
| cryptex-it-tools | Developer utilities — regex, JSON, hash, encode, etc. (tools.DOMAIN) |
| cryptex-notes | Quartz static site — PKM vault rendered as HTML (notes.DOMAIN) |
| cryptex-quartz-builder | node:22-alpine build container — converts PKM vault → Quartz HTML (cron every 15min, restart:no) |
| tailscale (host) | Backup SSH access + WebDAV direct access — host-installed, not a container |
...
code.<DOMAIN>           → http://172.18.0.1:8082  (Zellij web — host service via socat proxy on Docker bridge)
...
pdf.<DOMAIN>            → cryptex-stirling-pdf:8080  (Java — 60s boot, 502 during startup is normal)
files.<DOMAIN>          → cryptex-rclone:8080
tools.<DOMAIN>          → cryptex-it-tools:80
notes.<DOMAIN>          → cryptex-notes:80  (Quartz static site)
aqua.<DOMAIN>           → cryptex-portfolio:80  (AquaSoul Studio)
public.<DOMAIN>         → cryptex-portfolio:80  (nginx 301 → status.psidex.com/status/public)
portfolio.<DOMAIN>      → cryptex-portfolio:80  (nginx 301 → status.psidex.com/s/prt)
```
...
data/
  ...
  (Claude Code auth lives at /home/ubuntu/.claude on the host — not in data/)
  moodle-uploads/               # SCORM zips + import script (shared n8n ↔ moodle volume)
  pkm/                          # PKM vault — source of truth on VPS. Claude Code: $PKM or /opt/cryptex/data/pkm
  quartz-app/                   # Quartz install (node_modules etc) — persistent across builds
  quartz-output/                # Built static HTML — served by cryptex-notes nginx
```
...
- **Custom builds** — moodle, traxlrs use local Dockerfiles; don't use `docker pull` for these. n8n uses official image — tools (qpdf/pdftotext) are in `n8n_tools` named volume via init container
- **Zellij terminal** — host-installed (not a container). Binary at `/usr/local/bin/zellij`. Two systemd services: `zellij-web.service` (binds 127.0.0.1:8082) + `zellij-proxy.service` (socat forwards 172.18.0.1:8082 → 127.0.0.1:8082). Proxy needed because Zellij requires SSL to bind outside loopback, but cloudflared is a container and can't reach host 127.0.0.1. Cloudflare tunnel route: `http://172.18.0.1:8082`. Update: download new binary → replace → `sudo systemctl restart zellij-web zellij-proxy`
- **Zellij tools** — Claude Code + Gemini CLI installed globally via npm (`sudo npm update -g @anthropic-ai/claude-code @google/gemini-cli`). Shell tools: yazi, zoxide, starship, fzf, ripgrep, bat, fd via apt/binary.
- **Zellij auth** — token-based. Generate with `zellij web --create-token`. Claude auth persists in `~ubuntu/.claude`.
- **Zellaude plugin** — `/home/ubuntu/.config/zellij/plugins/zellaude.wasm` — Claude Code status bar (thinking/waiting/permission indicators)
