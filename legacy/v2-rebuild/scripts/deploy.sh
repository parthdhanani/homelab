...
alias notes-build='docker run --rm -v /opt/cryptex/data/pkm:/vault:ro -v /opt/cryptex/data/quartz-output:/output -v /opt/cryptex/data/quartz-app:/app -w /app node:22-alpine sh -c "node --no-deprecation /app/quartz/bootstrap-cli.mjs build --directory /vault --output /output/site && chown -R 1001:1001 /output/site"'
SHELLENV
...
if [ ! -f "${QUARTZ_APP}/package.json" ]; then
    ...
    if [ -d "${COMPOSE_DIR}/data/pkm" ] && [ "$(ls -A "${COMPOSE_DIR}/data/pkm" 2>/dev/null)" ]; then
        ...
        docker run --rm \
            ...
            -w /app \
            node:22-alpine \
            sh -c "node --no-deprecation /app/quartz/bootstrap-cli.mjs build --directory /vault --output /output/site && chown -R 1001:1001 /output/site" \
...
*/15 * * * * docker run --rm -v /opt/cryptex/data/pkm:/vault:ro -v /opt/cryptex/data/quartz-output:/output -v /opt/cryptex/data/quartz-app:/app -w /app node:22-alpine sh -c 'node --no-deprecation /app/quartz/bootstrap-cli.mjs build --directory /vault --output /output/site && chown -R 1001:1001 /output/site' >> /var/log/cryptex-quartz.log 2>&1
