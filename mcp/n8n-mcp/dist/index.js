...
server.tool("list_workflows", "List all n8n workflows with their active status", {}, async () => {
    ...
    const rows = (data.data ?? []).map((w) => `[${w.id}] ${w.name} — ${w.active ? "active" : "inactive"}`);
    return { content: [{ type: "text", text: rows.join("\n") || "No workflows found." }] };
});
server.tool("get_workflow", "Get full JSON of a workflow by ID", { id: z.string().describe("Workflow ID") }, async ({ id }) => {
    const data = await n8nFetch(`/workflows/${id}`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("execute_workflow", "Trigger a workflow execution. Only works on active workflows with a webhook/manual trigger.", {
...
server.tool("get_executions", "Get recent workflow executions, optionally filtered by workflow ID", {
    ...
    const rows = (data.data ?? []).map((e) => `[${e.id}] workflow=${e.workflowId} status=${e.status} started=${e.startedAt}`);
    return { content: [{ type: "text", text: rows.join("\n") || "No executions." }] };
});
server.tool("create_workflow", "Create a new n8n workflow from a JSON definition. The workflow will be inactive by default.", {
...
server.tool("update_workflow", "Update an existing workflow. Provide the full updated nodes + connections.", {
...
server.tool("set_workflow_active", "Activate or deactivate a workflow", {
    id: z.string().describe("Workflow ID"),
...
server.tool("delete_workflow", "Permanently delete a workflow by ID", { id: z.string().describe("Workflow ID") }, async ({ id }) => {
